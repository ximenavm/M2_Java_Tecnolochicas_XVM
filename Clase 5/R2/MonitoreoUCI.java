        ...
        return generarSignosVitales()
                .flatMap(sv -> {
                    ...
                    if (sv.fc < 50 || sv.fc > 120) {
                        eventos = eventos.concatWith(Flux.just("Paciente " + idPaciente + " - FC crítica: " + sv.fc + " bpm"));
                    } else if (sv.paSist > 140 || sv.paDiast > 90 || sv.paSist < 90 || sv.paDiast < 60) {
                        eventos = eventos.concatWith(Flux.just("Paciente " + idPaciente + " - PA crítica: " + sv.paSist + "/" + sv.paDiast + " mmHg"));
                    } else if (sv.spo2 < 90) {
                        eventos = eventos.concatWith(Flux.just("Paciente " + idPaciente + " - SpO2 baja: " + sv.spo2 + "%"));
                    }
